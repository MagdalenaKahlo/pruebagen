...
if(isset($_SESSION["matricula"]) and isset($_SESSION['tipo_usuario'])){
	if($_SESSION['tipo_usuario']==1){
		if(isset($_REQUEST['ELIMINAR'])){
			...
			$query="DELETE FROM preguntasfv WHERE id_preguntafv='".$_GET['cn_pre']."'";
			$result = $con-> query($query);
			mysqli_close($con);
			header("location:operacionpf.php?cn_materia=".$_REQUEST['cn_materia']);
		}
		if(isset($_POST['AGREGARPRE'])){
			require("../../entidades/Bases de datos/conexion.php");
			$query="INSERT INTO `generatest`.`preguntasfv` (`id_materia`, `preguntafv`, `respuestafv`, `id_profesor`) VALUES ('".htmlentities($_REQUEST['cn_materia'])."', '".htmlentities($_REQUEST['cn_pregunta'])."', '".htmlentities($_REQUEST['cn_resp'])."', '".htmlentities($_SESSION["matricula"])."');";
			$result = $con-> query($query);
			mysqli_close($con);
			header("location:operacionpf.php?cn_materia=".$_REQUEST['cn_materia']);		
		}else{?>
...
<table width="1000" border="0" align="center">
  <tbody>
    <tr>
      <td colspan="6" align="center"><font face="Impact" color="#323232" size="6">Registrar Pregunta de Falso o Verdadero</font><br/><br/></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td align="center" width="100"><input type="number" class="form-control" name="cn_resp" min="0" max="1" required></td>
    </tr>
    <tr>
...
<table width="600" border="1" align="center" bordercolor="#1DA8D3">
  <tbody>
    ...
    <?php
	require("../../entidades/Bases de datos/conexion.php");
	$query="SELECT * FROM preguntasfv where id_materia=".$_REQUEST['cn_materia']." and id_profesor='".$_SESSION["matricula"]."'";
	$result = $con-> query($query);
